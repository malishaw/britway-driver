  ...
  const mapDriverData = (): DriverDetail[] => {
    return [
      ...
      // {label: "Company VAT Number", value: driverData?.personalData?.companyVatNumber},
      {
        ...
        value: driverData?.otherData?.nationalInsuranceNumber,
      },
      ...
      { label: "Insurance Number", value: driverData?.otherData?.insurance },
      {
        ...
        value: driverData?.otherData?.insuranceExpiryDate,
      },
      {
        label: "Driving Licence Number",
        value: driverData?.otherData?.drivingLicence,
      },
      {
        label: "Driving Licence Expiry Date",
        value: driverData?.otherData?.drivingLicenceExpiryDate,
      },
      { label: "PCO Licence Number", value: driverData?.otherData?.PCOLicence },
      {
        label: "PCO Licence Expiry Date",
        value: driverData?.otherData?.PCOLicenceExpiryDate,
      },
      { label: "MOT", value: driverData?.otherData?.MOTLicence },
      {
        label: "MOT Expiry Date",
        value: driverData?.otherData?.MOTLicenceExpiryDate,
      },
      { label: "PHV Licence Number", value: driverData?.otherData?.PHVLicence },
      {
        label: "PHV Licence Expiry Date",
        value: driverData?.otherData?.PHVLicenceExpiryDate,
      },
      {
        label: "Driver Activity Status",
        value: driverData?.otherData?.driverActivityStatus,
      },
      {
        label: "Driver Licence Address Status",
        value: driverData?.otherData?.driverAddressStatus,
      },
      { label: "DBS Check Status", value: driverData?.otherData?.bgsStatus },
      {
        label: "Last Checked Date",
        value: driverData?.otherData?.lastCheckedDate,
      },
    ];
  };
    ...
    <div className="w-full">
      <div className="container mx-auto bg-white p-8 w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 ">
          ...
          <div>
            {mapDriverData()
              .slice(Math.ceil(mapDriverData().length / 2))
              .map((detail, index) => (
                <DetailComponent
                  key={index}
                  label={detail.label}
